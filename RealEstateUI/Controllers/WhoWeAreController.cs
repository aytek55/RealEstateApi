...
using RealEstateUI.Dtos.WhoWeAreDetailDtos;
using System.Text;
...
namespace RealEstateUI.Controllers
{
	public class WhoWeAreController : Controller
	{
		...
		public async Task<IActionResult> CreateWhoWeAreDetail(CreateWhoWreAreDetailDto createWhoWreAreDetailDto)
		{
			...
			var responseMessage = await client.PostAsync("https://localhost:7197/api/WhoWeAreDetail", stringContent);
			if (responseMessage.IsSuccessStatusCode)
			{
		...
		{
			var client = _httpClientFactory.CreateClient();
			var reponseMessage = await client.DeleteAsync($"https://localhost:7197/api/WhoWeAreDetail/{id}");
		...
		{
			var client = _httpClientFactory.CreateClient();
			var responseMessage = await client.GetAsync($"https://localhost:7197/api/WhoWeAreDetail/{id}");
			if (responseMessage.IsSuccessStatusCode)
			{
				var jsonData = await responseMessage.Content.ReadAsStringAsync();
		...
		public async Task<IActionResult> UpdateWhoWeAreDetail(UpdateWhoWeAreDetailDto updateWhoWeAreDetailDto)
		{
			...
			StringContent stringContent = new StringContent(jsonData, Encoding.UTF8, "application/json");
			var responseMessage = await client.PutAsync("https://localhost:7197/api/WhoWeAreDetail/", stringContent);
			if (responseMessage.IsSuccessStatusCode)
			{
				return RedirectToAction("Index");
			}
